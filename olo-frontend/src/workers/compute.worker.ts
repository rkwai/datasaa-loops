...
ctx.onmessage = (event: MessageEvent<ComputeJobPayload>) => {
  runCompute(event.data).catch((error) =>
    ctx.postMessage({ type: 'error', message: error.message } satisfies ComputeWorkerResponse),
  )
}

async function runCompute(payload: ComputeJobPayload) {
  ...
  await postProgress('computing customer metrics')
  ...
  await postProgress('computing channel metrics')
  ...
  await postProgress('writing materialized tables')
  ...
  ctx.postMessage({
    type: 'completed',
    customerCount: customerMetrics.length,
    channelCount: channelMetrics.length,
  } satisfies ComputeWorkerResponse)

  async function postProgress(phase: string) {
    ctx.postMessage({ type: 'progress', phase } satisfies ComputeWorkerResponse)
    await db.jobs.update(payload.jobId, {
      phase,
      updatedAt: new Date().toISOString(),
    })
...
function assignSegments(
  ...
  const ltvs = Array.from(totals.values())
    .map((entry) => entry.ltv)
    .sort((a, b) => a - b)
  const highIdx = Math.max(ltvs.length - 1, 0) * (config.segmentHighQuantile ?? 0.9)
  const midIdx = Math.max(ltvs.length - 1, 0) * (config.segmentMidQuantile ?? 0.7)
  const highThreshold = ltvs[Math.floor(highIdx)] ?? 0
  const midThreshold = ltvs[Math.floor(midIdx)] ?? 0
